...
class Pager
{
	...
	*/
	 ...
	 * $sql = $myPager->loadSQL($sql,$iConn);  //adapt existing SQL statement
	 * </code>
	 *
	 ...
	 * @param MySQLi connection $iConn is an established MySQLi connection	 
	 * @return string The adapted SQL statement to be used by the page.
	 */
	
	public function loadSQL($sql,$iConn)
	{//SQL statement must be loaded to extract the numrows
		...
		if(!$findFrom){die(myerror(__FILE__,__LINE__,"SQL statement must include a single space on each side of the SQL keyword ' from '"));}
		...
		$result = mysqli_query($iConn,$rowsql) or die(myerror(__FILE__,__LINE__,mysqli_error($iConn)));
		list($this->numrows) = mysqli_fetch_row($result) or die(myerror(__FILE__,__LINE__,mysqli_error($iConn)));
